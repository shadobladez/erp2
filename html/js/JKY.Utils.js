...
//		in Logoff Page, it cannot set cookie [JKY.session_time]
		if (typeof(jky_program) !== 'string' || jky_program !== 'log_off') {
...
JKY.reset_all_events = function() {
	JKY.set_all_events		= function()				{return;};
	JKY.set_initial_values	= function()				{return;};
	JKY.display_list		= function()				{return;};
	JKY.set_table_row		= function(the_row)			{return;};
	JKY.display_form		= function()				{return;};
	JKY.display_replace		= function()				{return;};
	JKY.set_form_row		= function(the_row)			{return;};
	JKY.process_add_new		= function()				{return;};
	JKY.set_add_new_row		= function()				{return;};
	JKY.get_form_set		= function()				{return;};
	JKY.combine_row			= function(the_id)			{return;};
	JKY.print_row			= function(the_id)			{return;};
	JKY.approve_row			= function(the_id)			{return;};
	JKY.close_row			= function(the_id)			{return;};
	JKY.replace_row			= function(the_id)			{return;};

	JKY.process_start		= function(the_id)			{return;};
	JKY.process_insert		= function(the_id)			{return;};
	JKY.process_update		= function(the_id, the_row)	{return;};
	JKY.process_copy		= function(the_id, the_row)	{return;};
	JKY.process_delete		= function(the_id, the_row)	{return;};
	JKY.process_combine		= function(the_ids)			{return;};
	JKY.process_publish		= function()				{return;};
 ...
 * @param	the_id
 */
JKY.process_application = function(action, the_id) {
...
//		setTimeout(function()	{$('#jky-loading').hide();}, 1000);		//	for Colors 2000
		setTimeout(function()	{$('#' + id_name).css('display', 'none');}, 100);
	}else{
		$('#' + id_name).hide();
	}
};
...
//	JKY.set_html('jky-company-logo', '<img src="/img/' + company_logo + '" />');
	JKY.set_src('jky-company-logo', '/img/' + company_logo);
};
...
JKY.is_permitted = function(the_menu_id) {
	var my_resource = '';
	switch(the_menu_id) {
		case('sales'		)	:	my_resource = 'Menu-Sales'		; break;
		case('planning'		)	:	my_resource = 'Menu-Planning'	; break;
		case('production'	)	:	my_resource = 'Menu-Production'	; break;
		case('threads'		)	:	my_resource = 'Menu-Threads'	; break;
		case('boxes'		)	:	my_resource = 'Menu-Boxes'		; break;
		case('dyers'		)	:	my_resource = 'Menu-Dyers'		; break;
		case('pieces'		)	:	my_resource = 'Menu-Pieces'		; break;
		case('receiving'	)	:	my_resource = 'Menu-Receiving'	; break;
		case('fabrics'		)	:	my_resource = 'Menu-Fabrics'	; break;
		case('help'			)	:	my_resource = 'Menu-Help'		; break;
		case('admin'		)	:	my_resource = 'Menu-Admin'		; break;
		case('support'		)	:	my_resource = 'Menu-Support'	; break;
...
JKY.process_log_off = function() {
	...
	JKY.ajax(false, my_data, function() {
		JKY.set_html('jky-application', '');
		JKY.set_html('jky-side-bar'   , '');
		JKY.load_dialog('log_off', 234);
	})
}
